...
# Data source to get the cluster's primary security group
...
resource "aws_security_group" "maestro_db" {
  ...
  # Allow from additional security group (provided by module)
  ingress {
  ...
  # Allow from cluster primary security group (used by Auto Mode nodes)
  ingress {
    description     = "PostgreSQL from EKS cluster primary security group"
    from_port       = 5432
    to_port         = 5432
    protocol        = "tcp"
