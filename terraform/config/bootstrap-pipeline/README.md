...
The script will:
1. Create Terraform state infrastructure (S3 + DynamoDB)
...
This creates:
- S3 bucket: `terraform-state-{ACCOUNT_ID}`
...
2. Find connections in **PENDING** state:
   - `central-github-connection` (Regional pipeline)
   - `regional-github-connection` (Management pipeline)
3. Click **"Update pending connection"** for each
4. Authorize with GitHub

## What Gets Created

### Regional Cluster Pipeline Resources

- **CodePipeline**: `central-regional-pipeline`
- **CodeBuild Projects**:
  - `regional-cluster-validate`
  - `regional-cluster-apply`
  - `regional-cluster-bootstrap`
- **GitHub Connection**: `central-github-connection`
- **S3 Artifact Bucket**: `central-pipeline-artifacts-*`
- **IAM Roles**: CodePipeline and CodeBuild service roles

### Management Cluster Pipeline Resources

- **CodePipeline**: `regional-management-pipeline`
- **CodeBuild Project**: `regional-management-provisioner`
- **GitHub Connection**: `regional-github-connection`
- **S3 Buckets**:
  - Artifact bucket: `regional-pipeline-artifacts-*`
  - State bucket: `terraform-state-management-{ACCOUNT_ID}`
- **DynamoDB Table**: `terraform-locks-management`
- **IAM Roles**: CodePipeline and CodeBuild service roles
...
**Solution**: Force unlock with the lock ID from the error message:
```bash
